                                ...
                                <ul>
                                    @foreach($subCategories as $sub)
                                        @if($sub->parent == $category->id)
                                            <li><a style="color: #D3BC8D;"
                                                   href="{{url('servicos/'.$sub->slug.'/'.$sub->id)}}">+ {{($sub->menu_name != '' ? $sub->menu_name : $sub->title)}}</a>
                                            </li>
                                            @foreach($services as $service)
                                                <div class="services_submenu">
                                                    @if($service->category_id == $sub->id)
                                                        <a href="{{url('/servico/'.$service->slug)}}">+ {{($service->menu_name != ''  ? $service->menu_name : $service->title)}}</a>
                                                    @endif
                                                </div>
                                            @endforeach
                                        @endif
